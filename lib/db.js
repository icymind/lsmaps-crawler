    ...
    for (let i = 0; i < tradesArray.length; i += 1) {
      try {
        ...
        if (parseInt(err.code) === 23505) {
          // duplicate key value violates unique constraint "computer_pkey"
          console.warn(`trade with id: ${tradesArray[i].Id} already existed in database, skipped it.`)
        } else {
          console.log(err.code)
          console.log(err.message)
          console.error(`can't save trade with id: ${tradesArray[i].Id} to db.`)
        }
